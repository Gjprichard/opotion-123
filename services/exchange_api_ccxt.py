...
def get_underlying_price(symbol, exchange_id='deribit', max_retries=3, retry_delay=1):
    """
    获取底层资产的当前价格，带有重试机制
    ...
    while retry_count < max_retries:
        try:
            ...
            if not exchange:
                logging.error(f"初始化{exchange_id}交易所失败")
                retry_count += 1
                time.sleep(retry_delay)
                continue

            # 使用ticker获取当前价格
            ticker_symbol = f"{symbol}-PERPETUAL"  # 使用永续合约获取价格
            ticker = exchange.fetch_ticker(ticker_symbol)

            if ticker and 'last' in ticker:
                return ticker['last']
            else:
                logging.warning(f"从{exchange_id}获取{symbol}价格失败，尝试重试 {retry_count+1}/{max_retries}")
                retry_count += 1
                time.sleep(retry_delay)

        except Exception as e:
            last_error = str(e)
            logging.warning(f"获取{symbol}价格时发生错误 (尝试 {retry_count+1}/{max_retries}): {last_error}")
            retry_count += 1
            time.sleep(retry_delay)

    # 所有重试都失败后记录错误
    logging.error(f"在{max_retries}次尝试后获取{symbol}价格失败: {last_error}")
    return None
